...
$user_name = "";
if (isset($user_id)) {
    $sql = "SELECT std_Fname FROM freelance WHERE std_id = $user_id";
    $result = $connection->query($sql);

    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $user_name = $row['std_Fname'];
    }
}
...
<body>
    <nav class="fixed-top">
        ...
        <ul>
            <li><a class="active" href="freelanceindex.php">Home</a></li>
            <li><a href="Board.php">บอร์ดประกาศงาน</a></li>
            <li><a href="history.php">ประวัติการประกาศงาน</a></li>
            <li class="dropdown">
    ...
    <div class="dropdown-content">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<a href="jobcategory.php?skill=' . urlencode($skill_name) . '">' . $skill_name . '</a>';
            }
        } else {
